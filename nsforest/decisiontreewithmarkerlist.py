...
def DecisionTree(adata, cluster_header, medians_header, binary_scores_header, 
                 marker_genes_dict={}, beta=0.5, exact = False, 
    ...
    for cl in cluster_list[:]:
        ...
        for marker in marker_genes_dict[cl]: 
            if marker in list(cluster_medians.columns): 
                ...
                print(f"cannot find {marker} in cluster_medians, check if marker is positively expressed")
        ...
        markers, scores, score_max = mydecisiontreeevaluation.myDecisionTreeEvaluation(adata, df_dummies, cl, marker_genes, beta, exact = exact)
        print("\t" + str(markers))
        print("\t" + str(score_max))
