...
def NSForest(adata, cluster_header, medians_header, binary_scores_header, 
    ...
    print("Saving number of genes selected per cluster as... " + output_folder + outputfilename + "_gene_selection.csv")
    ...
    for cl in tqdm(cluster_list, desc = "running NSForest on all clusters"): 
        ...
        df_results.to_csv(f"{output_folder}{outputfilename}_results.csv", index=False)
    ...
    on_target_ratio = calculate_metrics.on_target_ratio(adata, df_results, cluster_header, medians_header, output_folder, outputfilename)
