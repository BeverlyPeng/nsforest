...
def _parse_args(args, **kwargs): 
  ...
  parser.add_argument("-p", action='store_true', 
                      help="only calculating median and binary scores")
...
def str_to_list(values): 
    values = [val.replace("[", "").replace("]", "").replace(", ", ",") for val in values]
    values = [val.replace("'", "").replace('"', "").split(",") for val in values]
    return values

# Convering marker_genes_csv to dictionary (clusterName: marker_genes)
def prepare_markers(filename): 
    marker_genes_csv = pd.read_csv(filename) 
    if "[" in list(marker_genes_csv["NSForest_markers"])[0]: # "['NTNG1', 'EYA4']"
        marker_genes_dict = dict(zip(marker_genes_csv["clusterName"], str_to_list(marker_genes_csv["NSForest_markers"])))
    else: # 'NTNG1' \n 'EYA4'
        marker_genes_dict = {}
        for index, row in marker_genes_csv.iterrows(): 
            if row["clusterName"] in marker_genes_dict: 
                marker_genes_dict[row["clusterName"]].append(row["NSForest_markers"])
            else: 
                marker_genes_dict[row["clusterName"]] = [row["NSForest_markers"]]
...
def get_kwargs(filename): 
    ...
    if "output_folder" not in kwargs: 
        kwargs["output_folder"] = "NSForest_outputs_" + kwargs["outputfilename"] + "/"
    return kwargs

# Subsetting kwargs for specific function
def subset_kwargs(function, kwargs): 
    subset_kwargs = {}
    dictionary = {"nsf.NSF": ["cluster_header", "cluster_list", "medians_header", "binary_scores_header", 
                              "beta", "n_top_genes", "n_binary_genes", "n_genes_eval", "output_folder", "outputfilename"], 
